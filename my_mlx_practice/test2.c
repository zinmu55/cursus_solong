...
// プレイヤー移動処理のヘルパー関数
void	try_move_player(t_game *game, int new_x, int new_y)
{
	char	target_cell;
	...
	target_cell = game->map.data[new_y][new_x];
	// 壁との衝突判定
	if (target_cell == '1')
	{
		return ; // 壁なので移動不可
	...
	if (target_cell == 'E')
	{
		...
		{
			printf("Congratulations! You collected all items and reached the exit!\n");
			close_window(game); // 全てのアイテムを回収していればゲーム終了
		}
		else
		{
	...
	// 移動が許可された場合
	// プレイヤーの古い位置を床に戻す
	game->map.data[game->map.player_pos_y][game->map.player_pos_x] = '0';
	// プレイヤーの新しい位置を更新
...
int	handle_keypress(int keycode, t_game *game)
{
	...
	current_x = game->map.player_pos_x;
	current_y = game->map.player_pos_y;
	printf("Key pressed: %d\n", keycode);
	// ESCキー (macOS: 53, Linux: 65307)
	if (keycode == 53 || keycode == 65307)
	{
		close_window(game); // プログラムを終了
	}
	// Wキー (macOS: 13, Linux: 119)
	else if (keycode == 13 || keycode == 119)
	{
		try_move_player(game, current_x, current_y - 1);
	}
	// Sキー (macOS: 1, Linux: 115)
	else if (keycode == 1 || keycode == 115)
	{
		try_move_player(game, current_x, current_y + 1);
	}
	// Aキー (macOS: 0, Linux: 97)
	else if (keycode == 0 || keycode == 97)
	{
		try_move_player(game, current_x - 1, current_y);
	}
	// Dキー (macOS: 2, Linux: 100)
...
int	main(int argc, char **argv)
{
	...
	if (strlen(argv[1]) < 4 || strcmp(argv[1] + strlen(argv[1]) - 4,
			".ber") != 0)
	{
